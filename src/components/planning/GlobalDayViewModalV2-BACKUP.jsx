...
const GlobalDayViewModalV2 = ({
              ...
              <div key={dayIndex} className="day-schedule-card">
                ...
                <div className="employees-schedule">
                  {currentShopEmployees?.map(employee => {
                    ...
                    const schedules = getEmployeeSchedule(employee.id, day.dateKey);
  ...
  // Fonction pour fusionner les créneaux consécutifs (accessible par tous les onglets)
  const mergeConsecutiveSlots = (schedules) => {
    ...
    for (let i = 0; i < schedules.length; i++) {
      if (schedules[i] === true || schedules[i] === 1) {
        ...
        if (currentStart !== null) {
          ...
          // Calculer l'heure de fin en ajoutant l'intervalle au créneau de fin
          const endTime = format(addMinutes(parse(config.timeSlots[i-1], 'HH:mm', new Date()), config.interval), 'HH:mm');
    ...
    if (currentStart !== null) {
      const startTime = config.timeSlots[currentStart];
      // Pour le dernier créneau, calculer l'heure de fin correctement
      const lastSlotIndex = schedules.length - 1;
      const endTime = format(addMinutes(parse(config.timeSlots[lastSlotIndex], 'HH:mm', new Date()), config.interval), 'HH:mm');
      timeSlots.push({ start: startTime, end: endTime });
    }
              ...
              >
                ...
                <div className="employees-schedule">
                  ...
                  {(() => {
                    ...
                    // Ajouter les employés de la boutique actuelle
                    if (currentShopEmployees) {
                      currentShopEmployees.forEach(emp => allEmployees.add(emp));
                    }
                    
                    // Ajouter les employés des autres boutiques
                    if (planningData && planningData.shops) {
                      for (const shopId in planningData.shops) {
                        ...
                        if (shop.employees && Array.isArray(shop.employees)) {
                          shop.employees.forEach(emp => allEmployees.add(emp));
                        }
                      }
                    }
                    ...
                    let employeeShop = null;
                    let schedules = null;
                    let isOnLeave = false;
                    
                    // 1. Vérifier d'abord la boutique actuelle
                    schedules = getEmployeeSchedule(employee.id, day.dateKey);
                    if (schedules && Array.isArray(schedules) && schedules.some(slot => slot === true || slot === 1)) {
                      employeeShop = getShopDisplayName(selectedShop);
                    } else {
                      ...
                      if (planningData && planningData.shops) {
                        for (const shopId in planningData.shops) {
                          ...
                          if (shop.name !== selectedShop) { // Éviter de revérifier la boutique actuelle
                            ...
                            if (otherShopSchedule && Array.isArray(otherShopSchedule) && otherShopSchedule.some(slot => slot === true || slot === 1)) {
                              // Horaires valides trouvés dans cette boutique
